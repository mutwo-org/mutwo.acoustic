...
class LoudnessToAmplitudeTest(unittest.TestCase):
    def test_sone_to_phon(self):
        self.assertEqual(
            acoustic_converters.LoudnessToAmplitude._sone_to_phon(1),
            40,
        )
        self.assertEqual(
            acoustic_converters.LoudnessToAmplitude._sone_to_phon(2),
            50,
        )
        self.assertEqual(
            acoustic_converters.LoudnessToAmplitude._sone_to_phon(0.5),
            31.39434452534506,
        )

    def test_convert(self):
        converter = acoustic_converters.LoudnessToAmplitude()
